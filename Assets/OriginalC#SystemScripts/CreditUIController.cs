...
public class CreditUIController : MonoBehaviour
{
    ...
    /// 出力先ラベルのクラス
    /// </summary>
    CreditTextController _textController;

    SkipCreditButtonChecker _skipPrologueButtonChecker;
    ...
    private void Awake()
    {
        if (GetComponent<UIDocument>() != null)//UIDocumenを取得出来たらそのまま取得
        {
            ...
            this._textController = new CreditTextController(_uiDocument.rootVisualElement);
            this._skipPrologueButtonChecker = new SkipCreditButtonChecker(_uiDocument.rootVisualElement);
...
public class CreditTextController
{
    private UnityEngine.UIElements.Label _label;
    public CreditTextController(VisualElement root)
    {
        this._label = root.Q<UnityEngine.UIElements.Label>("Credit_Label");
...
public sealed class SkipCreditButtonChecker
{
    private readonly UnityEngine.UIElements.Button _button;
    private bool _calledTitleScene = false;

    public SkipCreditButtonChecker(VisualElement root)//Buttonの宣言があいまいなため
    {
